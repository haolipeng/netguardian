#ifndef NETGUARDIAN_CORE_PACKET_PIPELINE_H
#define NETGUARDIAN_CORE_PACKET_PIPELINE_H
...
/**
 * PacketPipelineConfig - 数据包处理管道配置
 *
 * 简化后的配置结构，具体功能的配置由各 Processor 管理
 */
struct PacketPipelineConfig {
    // 基本配置
    bool verbose = false;
...
/**
 * PacketPipeline - 数据包处理管道
 *
 * **设计理念：Pipeline 架构**
 *
 * PacketPipeline 是一个 **处理器编排器（Processor Orchestrator）**，其唯一职责是：
 * - 管理处理器管道（Pipeline）
 * - 依次调用各处理器处理数据包
 * - 提供统计信息访问接口
 *
 * **架构说明**：
 * - Pipeline 本身不关心具体的业务逻辑（如检测、解析等）
 * - 所有业务逻辑都封装在各个 Processor 中
 * - 检测引擎（DetectionProcessor）只是 Pipeline 中的一个处理器
 *
 * **优点**：
 * - 单一职责：只做编排，不做业务逻辑
...
public:
     ...
     * @param config 配置
     */
    explicit PacketPipeline(const PacketPipelineConfig& config = PacketPipelineConfig())
        : config_(config)
    ...
    void add_processor(PacketProcessorPtr processor) {
        if (running_) {
            throw std::runtime_error("Cannot add processor while pipeline is running");
        }
    ...
    bool initialize() {
        ...
        if (config_.verbose) {
            std::cout << "[INFO] Initializing packet pipeline with "
        ...
        if (config_.verbose) {
            std::cout << "[INFO] Packet pipeline initialized successfully\n\n";
        }
    ...
    void start() {
        if (!initialized_) {
            if (!initialize()) {
                throw std::runtime_error("Packet pipeline initialization failed");
            }
        }
        running_ = true;

        if (config_.verbose) {
            std::cout << "[INFO] Packet pipeline started\n";
        }
    }
    ...
    void stop() {
        ...
        if (config_.verbose) {
            std::cout << "[INFO] Packet pipeline stopped\n";
        }
    }
     ...
     * 获取统计信息（引用）
     */
    const PipelineStats& get_stats() const {
     ...
     * 获取统计信息快照（线程安全）
     */
    PipelineStatsSnapshot get_stats_snapshot() const {
...
#endif // NETGUARDIAN_CORE_PACKET_PIPELINE_H
